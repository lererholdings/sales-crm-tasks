...
describe('logFieldChanges', () => {
  ...
  it('writes task_id/account_id from the context param when provided', async () => {
    await logFieldChanges(
      'task_note',
      'note1',
      'user1',
      'created',
      { content: { from: null, to: 'hi' } },
      { taskId: 'task1', accountId: 'acc1' },
    )

    const [, params] = queryMock.mock.calls[0]
    expect(params[5]).toBe('task1')
    expect(params[6]).toBe('acc1')
  })

  it('defaults task_id/account_id to null when no context is given', async () => {
    await logFieldChanges('task_type', 'tt1', 'user1', 'updated', { active: { from: true, to: false } })
...
describe('withAudit', () => {
  ...
  it('passes the after row through deriveContext into the audit entry', async () => {
    queryMock
      .mockResolvedValueOnce({ rows: [{ id: 'n1', task_id: 't1', content: 'hi' }] }) // after-fetch
      .mockResolvedValueOnce({}) // audit insert

    const innerHandler = vi.fn(async (req, res) => {
      res.status(201).json({ id: 'n1', task_id: 't1', content: 'hi' })
    })
    const wrapped = withAudit(
      {
        table: 'task_notes',
    ...
    const auditCall = queryMock.mock.calls[1]
    expect(auditCall[1][5]).toBe('t1') // task_id
    expect(auditCall[1][6]).toBeNull() // account_id, not derived here
  })
