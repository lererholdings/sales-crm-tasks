...
// context: { taskId, accountId } — which task/account this change happened
// on, distinct from entityType/entityId (e.g. a task_note's entityId is the
// note's own id, not the task it belongs to). Both left null when the
// entity isn't task/account-scoped (task_type, user).
export async function logFieldChanges(entityType, entityId, userId, action, changes, context = {}) {
  const fields = Object.keys(changes)
  if (fields.length === 0) return

  await query(
    `INSERT INTO audit_log (entity_type, entity_id, user_id, action, changed_fields, task_id, account_id)
...
// deriveContext(afterRow) => { taskId, accountId } — lets each call site say
// where its task/account context comes from (e.g. task_notes.task_id is
// already on the row; a task's own account_id is too). Defaults to neither,
// for entities with no task/account relationship.
export function withAudit({ table, entityType, fields, idParam = 'id', deriveContext = () => ({}) }, handler) {
  return async (req, res, user) => {
    ...
    const context = deriveContext(after)
    if (req.method === 'POST') {
      await logFieldChanges(entityType, afterId, user.id, 'created', toCreatedChanges(pick(after, fields)), context)
    } else {
      const action = req.method === 'DELETE' ? 'deleted' : 'updated'
      await logFieldChanges(entityType, afterId, user.id, action, diffRows(before, after, fields), context)
    }
  }
}
