...
| Production (raw Vercel URL) | https://sales-crm-tasks-lerers-projects.vercel.app — works as an alias: `vercel.json` redirects any unprefixed path to `/sales-tasks/...` and rewrites it back to the real files/functions there, so it now serves correctly instead of breaking ([#25](https://github.com/lererholdings/sales-crm-tasks/issues/25), resolved) |
| Preview (per branch) | `https://sales-crm-tasks-git-<branch-slug>-lerers-projects.vercel.app` (branch names should stay lowercase-hyphenated for this to match) — unaffected by `VITE_BASE_PATH`, still serves at root. Note: the same `/sales-tasks` redirect/rewrite rules apply here too (they're project-wide, not environment-scoped), so opening a Preview URL's bare root also bounces to `/sales-tasks/` — harmless since Preview assets aren't prefix-built and still resolve at their real unprefixed paths, but the URL bar will show the extra segment |
