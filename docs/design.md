...
| Vercel Deployment Protection: **off entirely** (all environments), not scoped to Preview-only — revised from the row above at Milestone 10 go-live | Clerk's sign-up mode changed from public to restricted/invite-only, since Clerk's own hosted sign-in/sign-up page is now directly reachable by anyone with a Preview or Production URL, not just Vercel-team members | Per-environment scoping of Vercel Authentication ("Only Preview Deployments") turned out to require a paid Vercel plan — not available on the Hobby tier this project runs on. The two remaining options were: (a) turn protection off everywhere, accepting that Preview URLs become technically reachable (still gated by a real Clerk login to do anything) — the option already anticipated as acceptable by issue #1's own text; or (b) add all 5 real users to the Vercel team, which duplicates the access-gate system the project deliberately avoided having two of. Went with (a), matching the original design intent that Clerk alone should gate real usage. The tradeoff this reopens: with Vercel no longer blocking unauthenticated visitors from even reaching the app, Clerk's sign-up page being public would let anyone self-register — mitigated by switching Clerk's sign-up mode to invite-only before flipping Vercel protection off. |
