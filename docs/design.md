...
| Test auth bypass | `validateSession` (`api/_lib/auth.js`) accepts a header-based bypass gated by `TEST_AUTH_BYPASS_SECRET`, instead of minting real Clerk sessions for testing | Clerk session tokens are short-lived and only obtainable via a real browser login — there's no "personal access token" model to lean on. The bypass skips Clerk JWT verification only; DB lookup/auto-provisioning still run for real, so tests exercise real behavior. Safety boundary: the env var must never be set in Vercel's Production environment — if unset, the bypass branch is unreachable regardless of what headers a request sends. Set in `.env.local` (local dev), as a GitHub Actions repo secret (CI integration tests, `npm run test:integration`), and in Vercel's Preview environment only. |
